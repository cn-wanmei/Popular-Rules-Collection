...
from scripts.icon_system_v5 import ROOT, VARIANTS, RENDERERS, RENDERER_VERSION, discover_services_from_ir, discover_services_from_rule_index, render_pngs, sha256, validate_source
...
class IconSystemV5Tests(unittest.TestCase):
    ...
    def test_semantic_ir_v2_shape_is_supported(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text(json.dumps({'schema':'semantic_ir_v2','entities':{'services':['demo']},'views':{'services':{'demo':{'display_name':'Demo','provider':'demo'}}},'memberships':{'demo':['r1']},'rules':[{'id':'r1','type':'DOMAIN_SUFFIX','value':'demo.example.com'}]}),encoding='utf-8')
            rows=discover_services_from_ir(path)
            self.assertEqual(rows[0]['service_id'],'demo')
            self.assertIn('demo.example.com',rows[0]['domains'])

    def test_renderer_version_is_v5(self): self.assertEqual(RENDERER_VERSION,'prc-icon-renderer-v5.0.0')
    ...
    def test_seven_renderer_modules_are_distinct(self):
        ...
        for svg in rendered.values(): self.assertIn('<image',svg)
    ...
    def test_png_raster_contract(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for size in (24,64): self.assertGreater((Path(tmp)/paths[str(size)]).stat().st_size,100)

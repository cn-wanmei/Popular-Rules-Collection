...
def main():
    ...
    items=data.get("by_id",{}).get(args.service_id,[])
    if not items:
        print("service_id not found:",args.service_id)
        return 1
    result={"service_id":args.service_id,"release":data.get("release"),"matches":items,"count":len(items)}
    print(json.dumps(result,ensure_ascii=False,indent=2) if args.json else "\n".join(x["icon"]["raw"] for x in items))
    return 0

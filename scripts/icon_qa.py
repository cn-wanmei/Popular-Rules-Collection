...
def svg_meta(path):
 text=path.read_text(encoding="utf-8"); root=ET.fromstring(text); vb=root.attrib.get("viewBox",""); nums=[float(x) for x in re.split(r"[\s,]+",vb.strip()) if x] if vb else []; aspect=nums[2]/nums[3] if len(nums)==4 and nums[2]>0 and nums[3]>0 else None; title=""
 for c in root.iter():
  if c.tag.rsplit("}",1)[-1]=="title" and c.text:title=c.text.strip();break
 return {"bytes":path.stat().st_size,"viewBox":vb,"aspect":aspect,"title":title}
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--json",default="reports/icon_qa.json");args=ap.parse_args();doc=yaml.safe_load(MANIFEST.read_text(encoding="utf-8")) or {};icons=doc.get("icons") or {};report={"schema":"icon_qa_v1","icon_count":len(icons),"checks":{"source_svg":0,"png_64":0,"png_128":0,"png_256":0,"semantic":0,"payment_separation":0},"errors":[],"warnings":[]}
 for key,meta in sorted(icons.items()):
  ...
  try:sm=svg_meta(svg);report["checks"]["source_svg"]+=1
  ...
  for size in SIZES:
   ...
   try:wh=image_size(p);report["checks"][f"png_{size}"]+=1
   except Exception as exc:report["errors"].append(f"{key}: unreadable PNG {rel}: {exc}");continue
 ...
 out=ROOT/args.json;out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");print(json.dumps({"icons":report["icon_count"],"errors":len(report["errors"]),"warnings":len(report["warnings"]),"checks":report["checks"],"report":str(out)},ensure_ascii=False));return 1 if report["errors"] else 0

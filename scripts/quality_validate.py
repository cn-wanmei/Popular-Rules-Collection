...
def main() -> int:
    ...
    empty_generated: list[str] = []
    gen_root = ROOT / "generated"
    for client in CLIENTS:
        d = gen_root / client
        if not d.is_dir():
            warnings.append(f"missing client dir: generated/{client}")
            continue
        for f in d.iterdir():
            if f.is_file() and f.stat().st_size == 0:
                empty_generated.append(f"{client}/{f.name}")
                warnings.append(f"empty generated: {client}/{f.name}")
    ...
    large_sets: dict[str, int] = {}
    if DOMAINS.is_dir():
        for name, threshold in LARGE_HINTS.items():
            path = DOMAINS / f"{name}.txt"
            if not path.exists():
                continue
            try:
                n = sum(1 for line in path.open(encoding="utf-8", errors="replace") if line.strip())
            except OSError:
                continue
            large_sets[name] = n
            if n >= threshold:
                warnings.append(f"large-set {name}: {n} lines (threshold {threshold})")

    shown = warnings[: args.max_warn]
    print(f"[quality_validate] flags={len(warnings)} shown={len(shown)} empty_generated={len(empty_generated)}")
    for w in shown:
        print(f"  WARN  {w}")
    ...
    try:
        ...
        rep = {
            "flags": len(warnings),
            "shown": len(shown),
            "empty_generated": len(empty_generated),
            "empty_generated_samples": empty_generated[:20],

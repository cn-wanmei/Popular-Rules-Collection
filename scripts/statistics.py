...
def load_intentional_unmaterialized() -> dict[str, str]:
    """Load SSOT from config/intentional_unmaterialized.yaml; fall back to hardcoded."""
    ...
    if isinstance(services, dict) and services:
        ...
        for sid, meta in services.items():
            if isinstance(meta, dict):
                out[str(sid)] = str(meta.get("reason") or "intentional")
            else:
                out[str(sid)] = str(meta)
        return out
    return dict(_INTENTIONAL_FALLBACK)
...
def main() -> int:
    ...
    intentional = {
        sid: reason
        for sid, reason in intentional_ssot.items()

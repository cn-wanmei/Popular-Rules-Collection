#!/usr/bin/env python3
"""normalize.py — V2-semantic upstream → Universal Rule Schema
- Streaming parse for large adblock lists
- File→service mapping loaded from registry.yaml
- DOMAIN vs DOMAIN-SUFFIX preserved in sidecars (.exact.txt vs .txt)
"""
...
def _ip_registry_targets() -> set[str]:
    ...
    try:
        ...
        return set()
    return {str(s.get("maps_to")) for s in (doc.get("sources") or []) if s.get("maps_to")}
    ...
    for service_id, by_type in sorted(values.items()):
        ...
        doc = {
            ...
            "metadata": {
                ...
                "notes": "Generated by normalize.py V2-semantic",
                "stats": {
        ...
        # V2: never merge domain + domain_suffix into one semantic bucket
        exact_vals = set(by_type.get("domain", set()))
        suffix_vals = set(by_type.get("domain_suffix", set()))
        (DOMAINS / f"{service_id}.txt").write_text(
            "\n".join(sorted(suffix_vals)) + ("\n" if suffix_vals else ""),
            encoding="utf-8",
        )
        exact_path = DOMAINS / f"{service_id}.exact.txt"
        if exact_vals:
            exact_path.write_text("\n".join(sorted(exact_vals)) + "\n", encoding="utf-8")
        elif exact_path.exists():
            exact_path.unlink()

        ip_vals = set(by_type.get("ip_cidr", set())) | set(by_type.get("ip_cidr6", set()))
        if ip_vals:
            if service_id in _ip_registry_targets():
                print(
                    f"  skip ips overwrite {service_id}: ip_registry-owned "
                    f"(domain-source ips={len(ip_vals)})"
                )
            else:

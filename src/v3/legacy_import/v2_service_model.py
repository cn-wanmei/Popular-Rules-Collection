"""Load config/service_model into V3 EntityGraph (data only)."""
...
from src.v3.core.models.entity import AggregateView, EntityGraph, Group, Provider, Service
...
def load_entity_graph(sm_dir: Path) -> EntityGraph:
    ...
    for sid, s in (_load(sm_dir / "services.yaml").get("services") or {}).items():
        ...
        if isinstance(br, dict) and br.get("id"):
            ...
            body = sid[: -len("-core")]
        g.services[sid] = Service(
            id=sid, provider=s.get("provider"), display_name=s.get("display_name") or sid,
            body_service_id=body, meta=s,
        )
    ...
    for aid, a in (_load(sm_dir / "memberships.yaml").get("aggregates") or {}).items():
        a = a or {}
        g.aggregates[aid] = AggregateView(
            id=aid, members=list(a.get("members") or []), exclude=list(a.get("exclude") or []), meta=a,
        )

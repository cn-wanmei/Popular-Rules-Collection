"""Quarantine / promote lifecycle (observe mode from sources/health.yaml)."""
...
def set_status(state_path: Path, source_id: str, status: str, reason: str = "", meta: dict | None = None) -> dict:
    ...
    entry.update({
        ...
        "meta": meta or entry.get("meta") or {},
    })
...
def evaluate_health_yaml(root: Path, state_path: Path) -> dict:
    ...
    for item in items:
        ...
        if ok is False or item.get("status") in ("error", "fail", "blocked"):
            set_status(state_path, sid, "quarantined", reason=str(item.get("error") or "health_fail"))
            report["quarantined"] += 1
        else:
            set_status(state_path, sid, "accepted", reason="health_ok")
            report["accepted"] += 1
    return report

...
def _run_stage(name: str) -> int:
    ...
    from src.v3.ir.builder import build_ir
    ...
    if name == "ir":
        meta = build_ir(rules, memberships, graph, ir_dir)
        print(f"[v3] ir rules={meta['rules']} digest={meta['ir_digest'][:12]}")
        return 0
    ...
    if name == "diff":
        ...
        if v2_summary.exists() and v3_summary.exists():
            ...
            for vid, v3r in v3.items():
                v2r = v2.get(vid) or {}
                rows.append({
                    "view": vid, "v2_rules": v2r.get("rules"), "v3_rules": v3r.get("rules"),
                    "rules_match": v2r.get("rules") == v3r.get("rules"),
                    "sha_match": v2r.get("sha256") == v3r.get("sha256"),
                })
            diff = {"compared": True, "rows": rows, "all_rules_match": all(r["rules_match"] for r in rows)}
            print(f"[v3] diff all_rules_match={diff.get('all_rules_match')}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if stage == "all":
        for s in ("canonical", "hierarchy", "ir", "adapters", "diff"):

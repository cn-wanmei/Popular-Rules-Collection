"""Immutable snapshots under data/v3/snapshots/{id}/."""
...
def create_snapshot(root: Path, sources: list[Path], out_root: Path | None = None, label: str = "manual") -> dict:
    ...
    snap_id = f"{ts}-{label}"
    out_root = out_root or (root / "data" / "v3" / "snapshots")
...
def snapshot_v2_oracle(root: Path) -> dict:
    candidates = [
        root / "reports" / "hierarchy" / "summary.json",
        root / "reports" / "hierarchy" / "golden.json",
        root / "generated" / "ir" / "manifest.json",
        root / "config" / "service_model" / "memberships.yaml",
        root / "config" / "service_model" / "services.yaml",
        root / "config" / "service_model" / "providers.yaml",
    ]

"""Canonical Rule Store — streaming build under data/v3/canonical/."""
...
def build_from_v2_services(services_dir: Path, out_dir: Path) -> dict:
    ...
    seen: set[str] = set()
    n_rules = n_mem = 0
    with (out_dir / "rules.jsonl").open("w", encoding="utf-8") as fr, (
        out_dir / "service_rules.jsonl"
    ).open("w", encoding="utf-8") as fm:
        for p in sorted(services_dir.glob("*.yaml")):
            ...
            for r in doc.get("rules") or []:
                        ...
                        json.dumps(
                            {
                                ...
                                "type": typ,
                                "value": val,
                                "identity_key": identity_key(typ, val),
                                "provenance": {"sources": r.get("sources") or sources},
                ...
                n_mem += 1
    manifest = {
        "schema": "v3_canonical_store_v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "unique_rules": n_rules,
        "memberships": n_mem,
        "source": "v2_database_services_import",
    }

...
def validate_delete_authorization(
    ...
    *,
    ...
    if gate.get("status") != "PASS":
        raise LegacyDeleteError("final migration gate is not PASS")
    phase8 = gate.get("phase8") or {}
    if phase8.get("sot") != "v1_canonical":
        raise LegacyDeleteError("V1 Canonical is not the active Source of Truth")
    ...
    if boundary.get("automatic_delete") is not False:
        raise LegacyDeleteError("automatic deletion is not explicitly disabled")

    target = Path(target)
    if target.as_posix() != EXPECTED_TARGET.as_posix():
        raise LegacyDeleteError(f"refusing unexpected target: {target}")
    if target.is_symlink():
        raise LegacyDeleteError("refusing symlink deletion target")
    if not explicit_approval:
        raise LegacyDeleteError("explicit operator approval is required")
    if not target.exists():
        raise LegacyDeleteError("Legacy target does not exist")
    if not target.is_dir():
        raise LegacyDeleteError("Legacy target is not a directory")

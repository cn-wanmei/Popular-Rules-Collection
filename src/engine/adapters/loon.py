...
def render(rules: list[dict[str, Any]], out_path: Path) -> Path:
    ...
    if FMT != "list":
        raise ValueError(f"Loon adapter registry format must be list, got {FMT!r}")

    lines: list[str] = []
    for rule in rules:
        if not isinstance(rule, dict) or "type" not in rule or "value" not in rule:
            raise ValueError(f"Loon adapter received incomplete rule: {rule!r}")
        rule_type = str(rule["type"]).strip().upper().replace("_", "-")
        if rule_type not in SUPPORTED:
            raise ValueError(f"Unsupported Loon rule type: {rule_type!r}")
        value = str(rule["value"]).strip()
        if not value:
            raise ValueError(f"Loon adapter received empty rule value: {rule!r}")

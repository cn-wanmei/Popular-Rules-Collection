...
"""V3 adapter — migrated from scripts/build_mihomo.py. Output paths unchanged."""
#!/usr/bin/env python3
...
def bucket_counts(bucket: dict) -> int:
    return sum(len(bucket.get(k) or []) for k in ("domain", "domain_suffix", "domain_keyword", "domain_regex", "ip_cidr", "ip_cidr6"))
...
def payload_items(bucket: dict) -> list[str]:
    ...
    pure_domain = not domain_keyword and not domain_regex and not ip_cidr and not ip_cidr6
    if pure_domain:
        items: list[str] = []
        items.extend(domain)
